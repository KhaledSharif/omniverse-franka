#!/usr/bin/env python3
"""Evaluate trained PPO policy on Franka pick-and-place task.

This script loads a trained PPO policy and evaluates it on the Franka pick-and-place
environment, computing success rate and other metrics.

Usage:
    ./run.sh eval_policy.py models/ppo_franka.zip              # Evaluate with GUI
    ./run.sh eval_policy.py models/ppo_franka.zip --headless   # Headless evaluation
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate trained PPO policy on Franka pick-and-place task',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s models/ppo_franka.zip
      Evaluate with GUI for 10 episodes

  %(prog)s models/ppo_franka.zip --episodes 100 --headless
      Evaluate headless for 100 episodes

  %(prog)s models/ppo_franka.zip --stochastic
    ...
    print("=" * 60)
    print("Policy Evaluation for Franka Pick-and-Place")
    print("=" * 60)
    ...
    for episode in range(args.episodes):
        ...
        status = "SUCCESS" if task_complete else "FAILED"
