...
Usage:
    python replay_demo.py demos/recording.npz --info          # Show statistics only
    python replay_demo.py demos/recording.npz --episode 0     # Replay specific episode
    python replay_demo.py demos/recording.npz --speed 0.5     # Slow motion playback
    python replay_demo.py demos/recording.npz --successful    # Only replay successful episodes
"""
...
def visual_playback(filepath: str, episode_idx: int = None, speed: float = 1.0,
    ...
    franka = world.scene.add(
        Franka(prim_path="/World/Franka", name="replay_franka")
    )
    ...
    for ep_idx in episodes:
        ...
        for frame_idx, action in enumerate(actions):
            ...
            # Apply action (simplified - just move towards the observation state)
            # In a real implementation, you'd use the recorded joint positions
            # from the observations to set the robot state
